...
class Panel(ScreenPanel):
    ...
    def create_license_key_dialog(self, buttons=None, key=""):
        ...
        self.license_box = {}
        self.main_grid = Gtk.Grid(column_spacing=20, row_spacing=15, hexpand=True, vexpand=True)
        self.main_grid.set_margin_start(20)
        self.main_grid.set_margin_end(20)
        ...
        def add_labeled_value(row, label_text, value_text):
            ...
            self.main_grid.attach(label, 0, row, 1, 1)
            self.main_grid.attach(value, 1, row, 1, 1)
            return label, value
        ...
        key_box.pack_start(self.license_box["key_text"], False, False, 0)
        
        self.license_box["key_input"] = Gtk.Entry(hexpand=True, vexpand=False, width_chars=20)
        ...
        key_box.pack_start(self.license_box["key_input"], True, True, 0)

        self.main_grid.attach(key_box, 0, 5, 2, 1)
        image_size = min(self._gtk.content_width * 0.7, self._gtk.content_height * 0.35)
        image = self._gtk.Image("license", image_size, image_size)
        ...
        main_box.pack_start(image, False, True, 0)
        main_box.pack_start(self.main_grid, True, True, 0)
